...
function AttributePill({ block }: { block: Block }) {
    ...
    const segments = useMemo(
        () => parseAttributeSegments(block.name),
        [block.name],
    );
                            ...
                            {filtered.length === 0 ? (
                                ...
                                filtered.map((attr, idx) => (
                                    <button
                                        ...
                                        style={{
                                            paddingLeft: `${10 + attr.depth * 14}px`,
                                        }}
                                        className={`
...
function FunctionBlock({ block }: { block: Block }) {
    ...
    const argLabels =
        meta?.argLabels ?? block.args.map((_, i) => `Arg ${i + 1}`);
      ...
      `}
        ...
        `}
            >
                ...
                {isOperator ? (
                    <span
                        className={`font-bold text-base ${colors.text} font-mono`}
                    >
                        {block.name}
                    </span>
                ) : (
                    <>
                        ...
                        <span
                            className={`font-bold text-sm ${colors.text} tracking-wide`}
                        >
                            {displayLabel}
                        </span>
                    </>
            ...
            {!block.isCollapsed && (
                <div className="flex flex-col gap-2 p-3">
                    {block.args.map((arg, index) => {
                        ...
                        const isInVariadicRange =
                            isVariadic && index >= variadicStart;
                        const variadicSlotCount =
                            block.args.length - variadicStart;
            ...
            {block.isCollapsed && (
                <div className="px-3 py-2 text-xs text-slate-400 italic">
                    {block.args.filter(Boolean).length} / {block.args.length}{" "}
                    slots filled — click to expand
                </div>
            )}
        </div>
    );
